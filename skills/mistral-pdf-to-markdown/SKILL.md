...
The script requires:
- Mistral API key in environment variable `MISTRAL_API_KEY`
- Python packages: `mistralai`, `pypdf`
...
**API Key Not Found:**
```bash
Error: Mistral API key not found. Set MISTRAL_API_KEY env var.
```
→ 设置环境变量 `export MISTRAL_API_KEY=YOUR_KEY`
